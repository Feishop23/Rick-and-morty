...
import Pagi from './paginacion.css'
const Paginacion = ({pagina, setPagina, maximo}) => {
const [input, setInput] = useState(1)

  const nextpage = () =>{ 
    const next = parseInt(pagina) + 1
    const nextInput = parseInt(input) + 1
    if(next > maximo){
      setInput(1)
      setPagina(1)
    } else {
...
const reset = console.log("Stop")

return (
  <div className='container'>
    <button onClick={input === 1 ? reset : returnpage} className='previousPage'>
